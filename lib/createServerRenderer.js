...
var defaults = {
  entries: ['shared', 'app'],
  webpack_assetsPath: _path2['default'].resolve(__dirname, '../../../webpack-assets.json')
};
...
function createServerRenderer(_options) {
  ...
  return function app(req, res) {
    ...
    queue.await(function (err) {
      ...
      store.dispatch(_reduxRouterServer.match(req.originalUrl, function (err, redirect_location, routerState) {
        if (err) return sendError(res, err);
        ...
        var components = _lodash2['default'].uniq((always_fetch || {}).concat(routerState.components));
        ...
        _flReactUtils.fetchComponentData({ store: store, components: components }, function (err, fetch_result) {
          ...
          var js = _assets.jsAssets(options.entries, options.webpack_assetsPath);
          var script_tags = js.map(function (script) {
            return '<script type="application/javascript" src="' + script + '"></script>';
          }).join('\n');

          var css = _assets.cssAssets(options.entries, options.webpack_assetsPath);
